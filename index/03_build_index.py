...
Krok 3/3: Aplikuje váhy a sestaví finální index se dvěma variantami.
  ...
  FG_Equal  — průměr 7 komponent (1/7 každá)
  FG_OLS    — OLS koeficienty + intercept, ořezáno na [0, 100]
...
df['FG_Equal'] = X.dot(equal_weights)

# ── FG_OLS ────────────────────────────────────────────────────────────────────
ols_weights   = w.loc['OLS', COMP_COLS].values
ols_intercept = w.loc['OLS', 'intercept']
df['FG_OLS']  = (X.dot(ols_weights) + ols_intercept).clip(0, 100)

# ── Přehled ───────────────────────────────────────────────────────────────────
print("\nPlatné hodnoty per index:")
for col in ['FG_Equal', 'FG_OLS']:
    valid = df[col].dropna()
...
# ── Uložení ───────────────────────────────────────────────────────────────────
out_cols = ['SP500_Close'] + COMP_COLS + ['FG_Equal', 'FG_OLS', 'CNN_FearGreed']
